...
(function () {
  ...
  if (reduced || !('IntersectionObserver' in window)) revealEls.forEach(function (el) { el.style.opacity = '1'; el.style.transform = 'none'; });
  else { var io = new IntersectionObserver(function (entries) { entries.forEach(function (entry) { if (!entry.isIntersecting) return; var el = entry.target; var delay = el.dataset.revealDelay ? parseInt(el.dataset.revealDelay, 10) : 0; window.setTimeout(function () { el.style.transition = 'opacity var(--dur-entry) var(--ease-out-soft), transform var(--dur-entry) var(--ease-out-soft)'; el.style.opacity = '1'; el.style.transform = 'none'; }, delay); io.unobserve(el); }); }, { threshold: 0.12, rootMargin: '0px 0px -8% 0px' }); revealEls.forEach(function (el) { io.observe(el); }); }
  var sections = Array.prototype.slice.call(document.querySelectorAll('[data-nav-target]')); var links = Array.prototype.slice.call(document.querySelectorAll('.nav__links a'));
  ...
  function removeRedundantSectionControls() { document.querySelectorAll('.section[data-section-open]').forEach(function (section) { var trigger = section.querySelector(':scope > .container > .section__head .section__trigger'); if (trigger) { var heading = trigger.parentElement; trigger.replaceWith(document.createTextNode(trigger.textContent.trim())); if (heading) heading.normalize(); } var state = section.querySelector(':scope > .container > .section__head .section__state'); if (state) state.remove(); section.removeAttribute('data-section-open'); }); }
  function ensureThresholds() { ['about', 'creation', 'technology'].forEach(function (id) { var section = document.getElementById(id); if (!section || !section.parentNode) return; var next = section.nextElementSibling; if (next && next.classList.contains('threshold')) return; var threshold = document.createElement('div'); threshold.className = 'threshold threshold__glyph'; threshold.setAttribute('aria-hidden', 'true'); threshold.textContent = '· · ·'; section.parentNode.insertBefore(threshold, section.nextSibling); }); }
  function initExpansion() { removeRedundantSectionControls(); Array.prototype.slice.call(document.querySelectorAll('[data-subsection-open]')).forEach(function (subsection) { bindDisclosure(subsection); ensureSubsectionLabel(subsection); var trigger = subsection.querySelector('.subsection__trigger'); var contentId = trigger && trigger.getAttribute('aria-controls'); var content = contentId ? document.getElementById(contentId) : null; subsection.setAttribute('data-subsection-open', 'false'); if (trigger) trigger.setAttribute('aria-expanded', 'false'); if (content) { content.hidden = true; content.classList.remove('is-expanded'); } }); ensureThresholds(); }
  if (document.readyState === 'loading') document.addEventListener('DOMContentLoaded', initExpansion); else initExpansion();
  var showQrBtn = document.getElementById('show-qr-btn'); var qrModal = document.getElementById('upi-qr-modal'); var qrModalClose = qrModal ? qrModal.querySelector('.modal__close') : null; var qrModalOverlay = qrModal ? qrModal.querySelector('.modal__overlay') : null; var qrContainer = document.getElementById('upi-qr-code'); var upiDeepLink = 'upi://pay?pa=ankushisonline369%40okhdfcbank&pn=Ankush+Sarkar&tn=Thank+You%21&cu=INR';
  ...
  if (showQrBtn) showQrBtn.addEventListener('click', openQrModal); if (qrModalClose) qrModalClose.addEventListener('click', closeQrModal); if (qrModalOverlay) qrModalOverlay.addEventListener('click', closeQrModal); document.addEventListener('keydown', function (e) { if (e.key === 'Escape' && qrModal && !qrModal.hidden) closeQrModal(); });
})();
